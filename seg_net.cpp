...
//static const int OUTPUT_SIZE = 1000;  //直接输出conv的output,不需要指定size
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    //bn4
    IScaleLayer* bn5 = addBatchNorm2d(network, weightMap, *deconv1->getOutput(0), "bn5", 1e-5);
    //relu4
...
int main(int argc, char** argv)
{
    if (argc != 2) {
        ...
        std::cerr << "./seg_net -s   // serialize model to plan file" << std::endl;
        std::cerr << "./seg_net -d   // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("segNet.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("segNet.engine", std::ios::binary);
        if (file.good()) {
